class PageCommentTypesController < ApplicationController
    ...
    if has_permissions_or_redirect(:staff, page_comment_types_url)
      ...
      respond_to do |format|
        if !@page_comment_type.save
          flash[:notice] = 'PageCommentType was successfully created.' and return
  ...
  def upvote
    @voteable = PageCommentType.find(params[:id])
    page_comment_type.vote_for(@voteable)
    page_comment_type.add_points(1)
